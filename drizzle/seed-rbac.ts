...
async function seedRBAC() {
  ...
  const [adminRole] = await db
    .insert(roles)
    .values({ name: 'ADMIN', description: 'Administrator', isActive: true })
    .returning();
  // 2. Insert modules
  ...
  const [viewAction] = await db
    .insert(actions)
    .values({ name: 'VIEW', description: 'View' })
    .returning();
  // 5. Insert permissions
  const [viewUserPerm] = await db
    .insert(permissions)
    .values({
  ...
  await db
    ...
    .values({ roleId: adminRole.id, permissionId: viewUserPerm.id });
